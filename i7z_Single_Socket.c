...
void print_i7z_socket_single_stdout(
        struct cpu_socket_info socket_0,
        int PLATFORM_INFO_MSR,
        int PLATFORM_INFO_MSR_high,
        int PLATFORM_INFO_MSR_low,
        int* online_cpus,
        double cpu_freq_cpuinfo,
        struct timespec one_second_sleep,
        char TURBO_MODE,
        char* HT_ON_str,
        int* kk_1, U_L_L_I * old_val_CORE, U_L_L_I * old_val_REF, U_L_L_I * old_val_C3, U_L_L_I * old_val_C6, U_L_L_I * old_val_C7,
                             U_L_L_I * old_TSC,
        int estimated_mhz,  U_L_L_I * new_val_CORE,  U_L_L_I * new_val_REF,  U_L_L_I * new_val_C3,
                            U_L_L_I * new_val_C6,  U_L_L_I * new_val_C7,  U_L_L_I * new_TSC,
        double* _FREQ,
        double* _MULT,
        long double * C0_time,
        long double * C1_time,
...
// print_i7z_socket_single_stdout prints the output to stdout, does not use ncurses
void print_i7z_socket_single_stdout(
        struct cpu_socket_info socket_0,
        int PLATFORM_INFO_MSR,
        int PLATFORM_INFO_MSR_high,
        ...
        char TURBO_MODE,
        char* HT_ON_str,
        int* kk_1, U_L_L_I * old_val_CORE, U_L_L_I * old_val_REF, U_L_L_I * old_val_C3, U_L_L_I * old_val_C6, U_L_L_I * old_val_C7, U_L_L_I * old_TSC,
        int estimated_mhz,  U_L_L_I * new_val_CORE,  U_L_L_I * new_val_REF,  U_L_L_I * new_val_C3, U_L_L_I * new_val_C6,  U_L_L_I * new_val_C7,  U_L_L_I * new_TSC,
        double* _FREQ,
        double* _MULT,
        long double * C0_time,
        long double * C1_time,
        ...
        if(prog_options.i7_version.sandy_bridge || prog_options.i7_version.ivy_bridge || prog_options.i7_version.haswell){
            ...
            if (print_core[ii])
                printf("\tCore %d [%d]:\t  %0.2f (%.2fx)\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%d\t%0.4f\n\r",
                          ...
                          _FREQ[i],
                          _MULT[i],
                          THRESHOLD_BETWEEN_0_100(C0_time[i] * 100),
                          THRESHOLD_BETWEEN_0_100(c1_time),
                          THRESHOLD_BETWEEN_0_100(C3_time[i] * 100),
                          THRESHOLD_BETWEEN_0_100(C6_time[i] * 100),
                          Read_Thermal_Status_CPU(core_list[ii]),	//C0_time[i]*100+C1_time[i]*100 around 100
                          Read_Voltage_CPU(core_list[ii]));
            else
                printf("\tCore %d [%d]:\t  Garbage Values\n\r", ii + 1, core_list[ii]);
...
//	  print_socket_information(&socket_1);
    ...
    int run_time = 0;
    for (;;) {
        // increment the run time if run_times is set and do not eq to 0
        if (run_times != 0) {
           run_time++;
        }
        ...
        if(socket_0.max_cpu>1){
            socket_num=0;
            if(!use_ncurses) {
                print_i7z_socket_single_stdout(
                        socket_0,
                        PLATFORM_INFO_MSR,
                        PLATFORM_INFO_MSR_high,
                        ...
                        HT_ON_str,
                        &kk_1,
                        old_val_CORE[socket_num],
                        old_val_REF[socket_num],
                        old_val_C3[socket_num],
                        old_val_C6[socket_num],
                        old_val_C7[socket_num],
                        old_TSC[socket_num],
                        estimated_mhz,
                        new_val_CORE[socket_num],
                        new_val_REF[socket_num],
                        new_val_C3[socket_num],
                        new_val_C6[socket_num],
                        new_val_C7[socket_num],
                        new_TSC[socket_num],
                        _FREQ[socket_num],
                        _MULT[socket_num],
                        C0_time[socket_num],
        ...
        // check if the counter reached the limit
        if (run_times != 0 && run_time == run_times) {
            break;
        }
    }
}
